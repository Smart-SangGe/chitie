...
pub async fn check() -> Option<Finding> {
    ...
    // 检查 yum (RHEL/CentOS)
    if !is_vulnerable {
        if let Ok(output) = Command::new("yum").args(["list", "installed"]).output() {
            if output.status.success() {
        ...
        if let Ok(output) = Command::new("rpm").args(["-qa"]).output() {
            if output.status.success() {
                ...
                for line in rpm_output.lines() {
                    if line.contains("polkit")
                        && (line.contains("0.117-2") || line.contains("0.115-6"))
                    {
                        is_vulnerable = true;
                        break;
                    }
    ...
    .with_reference("https://github.blog/2021-06-10-privilege-escalation-polkit-root-on-linux-with-bug/");

    finding.details.push("ALERT: Vulnerable Polkit version detected!".to_string());
    finding.details.push("CVE-2021-3560: Race condition in Polkit authentication".to_string());
    finding.details.push("Impact: Unprivileged users can gain root privileges".to_string());
    finding.details.push("".to_string());
    finding.details.push("Vulnerable versions:".to_string());
    finding.details.push("  - polkit 0.105-26 (Ubuntu)".to_string());
    finding.details.push("  - polkit 0.117-2 (RHEL)".to_string());
    finding.details.push("  - polkit 0.115-6 (RHEL)".to_string());
    finding.details.push("".to_string());
    finding.details.push("Recommendation: Update Polkit immediately".to_string());
